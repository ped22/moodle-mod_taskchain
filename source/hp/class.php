...
/**
 * mod/taskchain/source/hp/class.php
 *
 * @package    mod
 ...
 * @subpackage taskchain
 */
class taskchain_source_hp extends taskchain_source {
     ...
     * @todo Finish documenting this function
     */
    public function html_get_entrytext() {
     ...
     * @todo Finish documenting this function
     */
    public function xml_get_name($textonly=true) {
        if (! isset($this->name)) {
            $this->name = '';
            $this->title = '';
     ...
     * @todo Finish documenting this function
     */
    public function xml_get_entrytext() {
        if (! isset($this->entrytext)) {
            $this->entrytext = '';
            ...
            if ($text = $this->xml_value($this->hbs_software.'-config-file,'.$this->hbs_tasktype.',exercise-subtitle')) {
                $this->entrytext .= '<h3>'.$text.'</h3>';
            }
            if ($text = $this->xml_value($this->hbs_software.'-config-file,'.$this->hbs_tasktype.',instructions')) {
                $this->entrytext .= '<div>'.$text.'</div>';
            }
        }
        return $this->entrytext;
    }
    ...
    public function xml_get_nexttask($context, $component, $filearea) {
        if (! isset($this->nexttask)) {
            ...
            if (! $this->xml_get_filecontents()) {
                // could not detect Hot Potatoes task type in xml file - shouldn't happen !!
                return false;
            }

            if (! $this->xml_value_int($this->hbs_software.'-config-file,global,include-next-ex')) {
                // next exercise is not enabled for this task
                return false;
            }
     ...
     * @todo Finish documenting this function
     */
    public function xml_get_filecontents() {
        if (! isset($this->xml)) {
            $this->xml = false;
            $this->xml_root = '';
     ...
     * @todo Finish documenting this function
     */
    public function xml_value($tags, $more_tags=null, $default='', $nl2br=true) {
        global $CFG;
     ...
     * @todo Finish documenting this function
     */
    public function single_line($match) {
        if (is_string($match)) {
            $before = '';
            $text   = $match;
            $after  = '';
     ...
     * @todo Finish documenting this function
     */
    public function xml_value_nl2br($match) {
        $before = $match[1];
        $text   = $match[2];
        $after  = $match[3];
     ...
     * @param xxx $tags
     * @param xxx $more_tags (optional, default=null)
     * @param xxx $default (optional, default=false)
     * @return xxx
     * @todo Finish documenting this function
     */
    public function xml_value_bool($tags, $more_tags=null, $default=false) {
     ...
     * @todo Finish documenting this function
     */
    public function xml_value_int($tags, $more_tags=null, $default=0) {
        $value = $this->xml_value($tags, $more_tags, $default, false);
        return intval($value);
    }
     ...
     * @todo Finish documenting this function
     */
    public function xml_value_js($tags, $more_tags=null, $default='', $nl2br=true, $convert_to_unicode=true) {
     ...
     * @todo Finish documenting this function
     */
    public function synchronize_moodle_settings(&$taskchain) {
        $name = $this->get_name();
        if ($name=='' || $name==$taskchain->name) {
            return false;
        } else {
            $taskchain->name = $name;
            return true;
