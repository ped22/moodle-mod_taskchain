...
class mod_taskchain_renderer extends plugin_renderer_base {
    ...
    public function header() {
        ...
        $heading = '';
        if (static::SHOW_HEADING) {
            if ($this->TC->taskchain) {
                $heading .= format_string($this->TC->taskchain->name);
                if ($this->TC->task) {
                    $heading .= get_string('labelsep', 'langconfig');
                    $heading .= format_string($this->TC->task->name);
                }
            }

            // Add edit icon for managers and teachers.
            if ($this->TC->can->manage() && empty($this->TC->inpopup)) {
                switch (true) {
                    case isset($this->TC->condition) : break; // do nothing
                    case isset($this->TC->task)      : $heading .= ' '.$this->taskedit_icon(); break;
                    case isset($this->TC->taskchain) : $heading .= ' '.$this->modedit_icon();  break;
                }
            }
        }

        // Moodle >= 4.x prints the activity heading as part of the header.
        // The activity header is generated by "magic_get_activityheader()"
        // in "lib/pagelib.php" using the mustache template at
        // "lib/templates/activity_header.mustache".

        // Set the start and end positions of the heading within the header.
        $start = $end = 0;

        // Insert/append $heading to $header.
        if (class_exists('\\core\\output\\activity_header')) {
            // In Moodle >= 4.x, a heading is already in the $header.
            // We want to locate it and replace it with $heading.
        ...
        if ($start === false || $end === false || $start >= $end) {
            // Moodle <= 3.11 (or a page without a heading).
            if (static::SHOW_HEADING) {
                $header .= parent::heading($heading);
            }
        } else {
            // Moodle >= 4.x (on a page with a heading).
            if (static::SHOW_HEADING) {
                // Move $start beyond the end of the opening tag.
                // This ensures we replace only the inner text.
                $start = strpos($header, '>', $start) + 1;
            } else {
                // Move $end beyond the end of the closing tag.
                // This ensures we remove both the tags and the inner text.
                $end = strpos($header, '>', $end) + 1;
            }
    ...
    function page_delete($message, $taskchainscriptname, $params, $footercourse='none') {
        ...
        // Note: the header includes the heading.
        $output .= $this->header();
